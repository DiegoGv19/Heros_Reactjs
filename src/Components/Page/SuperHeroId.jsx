...
import HeroInformation from './../Molecules/HeroInformation'
...
const SuperHeroId = ({match})=>{
    const Actual = superHeroList.filter(s => s.id === parseInt(match.params.id))[0]
    return(
            <>
                <img className="heroImage" src={Actual.Mainimage} alt={Actual.name}/>
                <HeroInformation name={Actual.name} nickName={Actual.nickName} description={Actual.description} imageTwo={Actual.imageTwo}/>
            </>
        )
}
